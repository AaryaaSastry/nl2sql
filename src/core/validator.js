...
export function validatePlan(plan, config) {
  if (!plan || typeof plan !== "object") {
    throw new McpError("Invalid plan object.", "VALIDATION_FAILED");
  }

  if (!plan.table || typeof plan.table !== "string") {
    throw new McpError("Plan must specify a base table.", "VALIDATION_FAILED");
  }

  ensureTable(config, plan.table);

  const columns = Array.isArray(plan.columns) ? plan.columns : [];
  const filters = Array.isArray(plan.filters) ? plan.filters : [];
  const aggregations = Array.isArray(plan.aggregations) ? plan.aggregations : [];
  const groupBy = Array.isArray(plan.groupBy) ? plan.groupBy : [];
  const having = Array.isArray(plan.having) ? plan.having : [];
  const joins = Array.isArray(plan.joins) ? plan.joins : [];

  validateColumns(config, plan.table, columns);
  validateFilters(config, plan.table, filters);
  validateAggregations(config, plan.table, aggregations);
  validateGroupBy(config, plan.table, groupBy);
  validateHaving(config, plan.table, having, plan);
  validateOrderBy(config, plan.table, plan);
  validateConsistency(plan);

  const resolvedJoins = expandAndValidateJoins({ ...plan, columns, filters, aggregations, groupBy, having, joins }, config);
  plan.joins = resolvedJoins;
